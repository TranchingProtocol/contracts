...
import type { IMicro } from "@zkamoeba/micro-web3/build/typechain";
import { IMicroFactory } from "@zkamoeba/micro-web3/build/typechain";
...
describe("L1ERC20Bridge tests", function () {
  ...
  before(async () => {
    ...
    // Exposing the methods of IMicro to the diamond proxy
    microContract = IMicroFactory.connect(diamondProxyContract.address, diamondProxyContract.provider);
  });
...
async function depositERC20(
  ...
  const neededValue = await microContract.l2TransactionBaseCost(gasPrice, l2GasLimit, gasPerPubdata);
