...
import "../micro/interfaces/IMicro.sol";
...
contract L1WethBridge is IL1Bridge, AllowListed, ReentrancyGuard {
    ...
    /// @dev micro smart contract that is used to operate with L2 via asynchronous L2 <-> L1 communication
    IMicro public immutable micro;
    ...
    /// @dev Used to indicate that micro L2 -> L1 WETH message was already processed
    mapping(uint256 => mapping(uint256 => bool)) public isWithdrawalFinalized;

    /// @dev Contract is expected to be used as proxy implementation.
    /// @dev Initialize the implementation to prevent Parity hack.
    constructor(address payable _l1WethAddress, IMicro _micro, IAllowList _allowList) reentrancyGuardInitializer {
    ...
    function deposit(
        ...
        txHash = micro.requestL2Transaction{value: _amount + msg.value}(
            l2Bridge,
            _amount,
            l2TxCalldata,
            _l2TxGasLimit,
    ...
    function finalizeWithdrawal(
        ...
        bool alreadyFinalised = micro.isEthWithdrawalFinalized(_l2BatchNumber, _l2MessageIndex);
        if (alreadyFinalised) {
            ...
            bool success = micro.proveL2MessageInclusion(_l2BatchNumber, _l2MessageIndex, l2ToL1Message, _merkleProof);
            require(success, "vq");
        } else {
            // Finalize the withdrawal if it is not yet done.
            micro.finalizeEthWithdrawal(_l2BatchNumber, _l2MessageIndex, _l2TxNumberInBatch, _message, _merkleProof);
        }
    ...
    receive() external payable {
        ...
        // 2. micro contract withdraw funds in `finalizeEthWithdrawal`
        require(msg.sender == l1WethAddress || msg.sender == address(micro), "pn");
