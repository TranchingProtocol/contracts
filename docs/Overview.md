...
L2 transactions on-chain and publishing the state transition diff. micro follows the second option.

See the [documentation](https://era.micro.io/docs/dev/fundamentals/rollups.html) to read more!
...
Technically, this L1 smart contract acts as a connector between Ethereum (L1) and micro (L2). This contract checks the
...
When the validator calls `commitBatches`, the same calldata will be propogated to the micro contract (`DiamondProxy` through
...
to the `proveBatches` function) will be propogated to the micro contract. After, the `delay` is elapsed, the validator
is allowed to call `executeBatches` to propogate the same calldata to micro contract.
...
See the
[documentation](https://era.micro.io/docs/dev/building-on-micro/contracts/contract-development.html#solidity-vyper-support)
to read more!
